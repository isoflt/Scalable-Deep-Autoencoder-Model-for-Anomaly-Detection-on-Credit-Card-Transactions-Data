...
*Just a note: The `requirements.txt` file contains two Tensorflow packages: tensorflow-macos and tensorflow-metal. tensorflow-macos specifically is only required if you have an Apple Silicon Mac, otherwise vanilla tensorflow will be fine. tensorflow-metal is a so-called PluggableDevice, which is just a GPU accelerator plugin for Macs using Apple Silicon. Also run tensorflow in a virtual environment to avoid weird issues.*
